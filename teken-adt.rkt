...
(define (maak-adt-teken titel h-pixels v-pixels)
  ...
  (define (verwijder-alles!)
    ...
    (define (verwijder-alle-aliens alienlijst) ;delete alle aliens
      (if (not (null? alienlijst))
          ...
          'ok))
    (verwijder-alle-aliens vloot-tiles)
    (verwijder-alle-kogels kogel-tiles)
    (verwijder-schip!)
    ((menu-laag 'add-drawable) menu-tile)) ;tekent het menu
  ...
  #|(define (maak-power-up-tile kleur)
    ...
    (cond
      ((eq? kleur 'geel)
       (set! tile (make-bitmap-tile "PowerUpGeel.jpg")))
      ((eq? kleur 'wit)
        (set! tile (make-bitmap-tile "PowerUpWit.jpg")))
      ((eq? kleur 'rood)
